...
function unauthorized(arg?: ErrorArg): EthereumRpcError<JsonRpcRequest<any>> {
  return ethErrors.provider.unauthorized({
    message:
      arg?.message ||
      'Unauthorized to perform action. Try requesting permission first using the `requestPermissions` method. More info available at https://github.com/MetaMask/rpc-cap',
    data: arg?.data || undefined,
  });
}

function methodNotFound(
  opts: MethodNotFoundArg,
): EthereumRpcError<JsonRpcRequest<any>> {
